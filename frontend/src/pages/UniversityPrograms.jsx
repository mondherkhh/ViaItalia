...
const Page = styled.div`
  --up-page-bg: #10121b;
  --up-surface: rgba(7, 18, 31, .76);
  --up-window: rgba(255, 255, 255, .035);
  --up-text: #f7fbff;
  --up-muted: #9eb1c6;
  --up-label: #f7fbff;
  --up-border: rgba(255, 255, 255, .1);
  --up-border-soft: rgba(255, 255, 255, .07);
  --up-input: rgba(7, 18, 31, .76);
  --up-accent: #73e6a4;
  --up-progress-track: #1b2935;

  min-height: 100%;
  padding: 1.5rem;
  background: radial-gradient(circle at 75% -10%, rgba(32, 201, 120, .08), transparent 34%), var(--up-page-bg);
  color: var(--up-text);
  ...
  body:not(.theme-dark) & {
    ...
    --up-muted: #475569;
    --up-label: #111111;
    --up-border: rgba(17, 17, 17, .14);
    --up-border-soft: rgba(17, 17, 17, .1);
    --up-input: #ffffff;
    --up-accent: #087f4d;
    --up-progress-track: #dce8e2;
  }
...
const StatCard = styled.div`
  ...
  span { display: block; color: var(--up-muted); font-size: .78rem; font-weight: 700; }
  strong { display: block; margin-top: .35rem; color: var(--up-text); font-size: 1.65rem; }
`;
...
const Select = styled.select`
  ...
  &:focus { border-color: #20c978; box-shadow: 0 0 0 3px rgba(22,163,107,.12); }
  option { background: var(--up-input); color: var(--up-text); }
`;
...
const WindowBox = styled.div`
  ...
  border-radius: 11px;
  background: var(--up-window);
  color: var(--up-muted);
  font-size: .78rem;
  line-height: 1.55;

  strong { color: var(--up-text); }
  a { color: #0e8a5a; }
`;
...
const TableWrap = styled.div`padding: 1.2rem; border: 1px solid var(--up-border); border-radius: 16px; background: var(--up-surface); box-shadow: 0 7px 22px rgba(29, 52, 75, .05);`;
...
const ModalCard = styled.div`width:min(720px,100%);max-height:86vh;overflow-y:auto;padding:1.35rem;border-radius:16px;background:var(--up-surface);border:1px solid var(--up-border);color:var(--up-text);box-shadow:0 24px 70px rgba(15,23,42,.22);`;
const Quote = styled.blockquote`margin:1rem 0;padding:.9rem 1rem;border-left:3px solid #20c978;background:var(--up-window);line-height:1.65;color:var(--up-muted);`;
...
function EvidenceAction({ e, onOpen }) {
  ...
  return <span><button type="button" onClick={() => onOpen(e)} style={{background:'rgba(19,184,92,.15)',color:'var(--up-accent)',border:'1px solid #bfe1cf',borderRadius:7,padding:'.3rem .45rem',cursor:'pointer'}}>Voir preuve · {Math.round((Number(e.confidence) || 0) * 100)}%</button>{href(e) && <><br/><a href={href(e)} target="_blank" rel="noreferrer" style={{fontSize:'.72rem'}}>Ouvrir source</a></>}</span>;
}
...
export default function UniversityPrograms() {
  ...
  return <Page>
    ...
    <FilterPanel>
      ...
      {progress && <div style={{color:'var(--up-accent)',marginTop:'1rem'}}>{progress.currentUniversity || 'Recherche officielle…'} — {progress.progress || 0}%<div style={{height:8,background:'var(--up-progress-track)',borderRadius:20,overflow:'hidden'}}><div style={{height:'100%',width:`${progress.progress || 0}%`,background:'#20c978'}} /></div></div>}
      {message && <p style={{marginTop:'1rem',color: message.includes('Échec') || message.includes('Impossible') ? '#c24141' : 'var(--up-accent)'}}>{message}</p>}
    </FilterPanel>

    <TableWrap>
      {loading ? <p style={{padding:'2rem',color:'var(--up-muted)'}}>Chargement…</p> : <ProgramsGrid>
        {programs.map(p => {
