...
import { FiVolume2, FiCreditCard, FiMessageSquare, FiFileText, FiTrendingUp } from 'react-icons/fi';
  ...
  return (
    <div style={styles.page}>
      ...
      <div className="client-dashboard-top" style={styles.grid}>
        <Panel icon={<FiVolume2 aria-hidden="true" />} title="Annonces">
          {data.announcements.length ? data.announcements.map(item => <div style={styles.item} key={item.id}><span aria-hidden="true"><FiVolume2 /></span><div><strong>{item.title || 'Annonce'}</strong><div style={styles.muted}>{item.content || item.message || ''}</div></div></div>) : <div style={{ ...styles.body, ...styles.muted }}>Aucune annonce disponible</div>}
        </Panel>
        <Panel icon={<FiCreditCard aria-hidden="true" />} title="Paiement">
      ...
      <div className="client-dashboard-middle" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 20 }}>
        <Panel icon={<FiMessageSquare aria-hidden="true" />} title="Messages"><div style={{ minHeight: 92 }}>{data.messages.length ? data.messages.slice(0, 3).map(item => <div style={styles.item} key={item.id}><span aria-hidden="true"><FiMessageSquare /></span><div><strong>{item.subject || item.title || 'Message'}</strong><div style={styles.muted}>{item.content || item.message || ''}</div></div></div>) : <div style={{ ...styles.body, ...styles.muted }}>Aucun message reçu</div>}</div></Panel>
        <Panel icon={<FiFileText aria-hidden="true" />} title="Contrat"><div style={{ ...styles.body, minHeight: 92 }}>{data.dossier ? <><strong>Contrat disponible</strong><div style={{ ...styles.muted, marginTop: 8 }}>Consultez votre dossier pour plus de détails.</div></> : <span style={styles.muted}>Aucun contrat disponible</span>}</div></Panel>
      </div>

      <Panel icon={<FiTrendingUp aria-hidden="true" />} title="Progression générale" style={{ marginBottom: 20 }}>
